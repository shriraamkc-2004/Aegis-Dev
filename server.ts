...
import { governanceService, type RetrievedDocument } from "./src/copilot/governance_service.js";
import { promptRegistry } from "./src/copilot/prompt_registry.js";
import { observabilityService } from "./src/copilot/observability.js";
import { caseManager, type CaseSeverity, type CaseStatus } from "./src/copilot/case_manager.js";
import { threatIntelService } from "./src/copilot/threat_intel.js";
import { aiEvaluationService } from "./src/copilot/evaluation.js";
import { disasterRecoveryService, RECOVERY_OBJECTIVES } from "./src/copilot/disaster_recovery.js";
import { healthMonitorService } from "./src/copilot/health_monitor.js";
import { processCopilotResponse, validateCopilotRequest, aiBoundaryMiddleware } from "./src/copilot/ai_boundary_middleware.js";
...
app.post("/api/copilot/chat", authenticateToken, async (req, res) => {
  try {
    ...
    const requestValidation = validateCopilotRequest(req.body.message || "", getOrgId(req.user!));
    ...
    observabilityService.recordApiLatency(latencyMs, "/api/copilot/chat", response.ok);
    ...
    if (response.ok && data.answer) {
      const sources: RetrievedDocument[] = (data.sources || []).map((s: any) => ({
        source: s.source || "",
        filename: s.filename || "",
        score: s.score || 0,
...
app.post("/api/copilot/ingest/all", authenticateToken, requireRole("super_admin", "org_admin", "demo_admin"), async (req, res) => {
  try {
    const response = await fetch(`${COPILOT_URL}/api/copilot/ingest/all`, { method: "POST" });
    res.json(await response.json());
  } catch (err: any) {
    res.status(503).json({ error: "Copilot service unavailable" });
  }
...
app.get("/api/copilot/audit", authenticateToken, requireRole("super_admin", "org_admin"), async (req, res) => {
  try {
...
app.post("/api/copilot/approve", authenticateToken, requireRole("super_admin", "org_admin", "soc_analyst"), async (req, res) => {
  try {
...
// ========================
app.get("/api/copilot/governance/status", authenticateToken, requireRole("super_admin", "org_admin", "soc_analyst"), (req, res) => {
  const status = governanceService.getStatus();
  res.json({ ...status, enabled: true });
});

app.get("/api/copilot/governance/audit", authenticateToken, requireRole("super_admin", "org_admin"), (req, res) => {
  const limit = parseInt(req.query.limit as string) || 50;
  res.json(governanceService.getAuditLog(limit));
});

app.get("/api/copilot/governance/approvals", authenticateToken, requireRole("super_admin", "org_admin", "soc_analyst"), (req, res) => {
  res.json(governanceService.getPendingApprovals());
});

app.post("/api/copilot/governance/approve", authenticateToken, requireRole("super_admin", "org_admin", "soc_analyst"), (req, res) => {
  const { response_id, approved, notes } = req.body;
  if (!response_id) { res.status(400).json({ error: "response_id is required." }); return; }
  ...
  logAudit(req.user!.id, req.user!.username, "GOVERNANCE_APPROVAL", "copilot", `${approved ? "Approved" : "Denied"} response ${response_id}`, req.ip || "", getOrgId(req.user!));
...
app.get("/api/copilot/prompts/:name", authenticateToken, async (req, res) => {
  try {
    const prompt = await promptRegistry.getActivePrompt(getOrgId(req.user!), req.params.name);
...
app.get("/api/copilot/prompts/:name/versions", authenticateToken, async (req, res) => {
  try {
    const versions = await promptRegistry.getPromptVersions(getOrgId(req.user!), req.params.name);
    res.json({ versions });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
app.post("/api/copilot/prompts", authenticateToken, requireRole("super_admin", "org_admin"), async (req, res) => {
  try {
    const { name, category, prompt_template, system_hint, variables } = req.body;
    if (!name || !prompt_template) { res.status(400).json({ error: "name and prompt_template are required." }); return; }
    ...
    logAudit(req.user!.id, req.user!.username, "CREATE_PROMPT", "prompts", `Created prompt: ${name} v${entry.version}`, req.ip || "", getOrgId(req.user!));
    res.json(entry);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
app.post("/api/copilot/prompts/:name/rollback", authenticateToken, requireRole("super_admin", "org_admin"), async (req, res) => {
  try {
    const { version } = req.body;
    if (!version) { res.status(400).json({ error: "version is required." }); return; }
    const result = await promptRegistry.rollbackToVersion(getOrgId(req.user!), req.params.name, version, req.user!.id);
    if (!result) { res.status(404).json({ error: "Prompt or version not found." }); return; }
    logAudit(req.user!.id, req.user!.username, "ROLLBACK_PROMPT", "prompts", `Rolled back ${req.params.name} to v${version}`, req.ip || "", getOrgId(req.user!));
    res.json(result);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
// ========================
app.get("/api/copilot/observability", authenticateToken, requireRole("super_admin", "org_admin"), (req, res) => {
  const tenantId = getOrgId(req.user!);
  res.json(observabilityService.getDashboard(tenantId === 0 ? undefined : tenantId));
});

app.get("/api/copilot/cost/summary", authenticateToken, requireRole("super_admin", "org_admin"), (req, res) => {
  const tenantId = getOrgId(req.user!);
  res.json(observabilityService.getCostSummary(tenantId === 0 ? undefined : tenantId));
});

app.get("/api/copilot/cost/report", authenticateToken, requireRole("super_admin", "org_admin"), (req, res) => {
  const startDate = (req.query.start as string) || new Date(Date.now() - 30 * 86400000).toISOString().slice(0, 10);
  const endDate = (req.query.end as string) || new Date().toISOString().slice(0, 10);
  const tenantId = getOrgId(req.user!);
  res.json({
    report: observabilityService.getCostReport(startDate, endDate, tenantId === 0 ? undefined : tenantId),
  });
});
...
app.get("/api/copilot/observability/metrics", authenticateToken, (req, res) => {
  ...
  if (!type) { res.status(400).json({ error: "type query parameter required." }); return; }
  res.json({ metrics: observabilityService.queryMetrics(type as any, since, getOrgId(req.user!), limit) });
});
...
app.get("/api/copilot/history/:sessionId", authenticateToken, async (req, res) => {
  try {
    const response = await fetch(`${COPILOT_URL}/api/copilot/history/${req.params.sessionId}`);
    res.status(response.status).json(await response.json());
  } catch (err: any) {
    res.status(503).json({ error: "Copilot service unavailable" });
  }
...
app.delete("/api/copilot/history/:sessionId", authenticateToken, async (req, res) => {
  try {
    const response = await fetch(`${COPILOT_URL}/api/copilot/history/${req.params.sessionId}`, { method: "DELETE" });
    res.status(response.status).json(await response.json());
  } catch (err: any) {
    res.status(503).json({ error: "Copilot service unavailable" });
  }
...
app.post("/api/copilot/history/:sessionId/messages", authenticateToken, async (req, res) => {
  try {
    const formData = new URLSearchParams();
    formData.append("role", req.body.role || "user");
    formData.append("content", req.body.content || "");
    if (req.body.sources) formData.append("sources", JSON.stringify(req.body.sources));
    if (req.body.confidence !== undefined) formData.append("confidence", String(req.body.confidence));
    formData.append("model_used", req.body.model_used || "");
    formData.append("reasoning", req.body.reasoning || "");
    formData.append("requires_approval", String(req.body.requires_approval || false));
    if (req.body.pending_action) formData.append("pending_action", req.body.pending_action);
    const response = await fetch(`${COPILOT_URL}/api/copilot/history/${req.params.sessionId}/messages`, {
      method: "POST",
...
// ========================
app.post("/api/copilot/upload", authenticateToken, requireRole("super_admin", "org_admin", "soc_analyst"), async (req, res) => {
  try {
    ...
    res.status(503).json({ error: "Copilot service unavailable", detail: err.message });
  }
});

app.post("/api/copilot/ingest/directory", authenticateToken, requireRole("super_admin", "org_admin"), async (req, res) => {
  try {
    const formData = new URLSearchParams();
    ...
    const response = await fetch(`${COPILOT_URL}/api/copilot/ingest/directory`, {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: formData.toString(),
    });
...
app.get("/api/cases", authenticateToken, async (req, res) => {
  try {
    ...
    const cases = await caseManager.listCases(tenantId === 0 ? 1 : tenantId, { status, severity });
    res.json(cases);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
app.post("/api/cases", authenticateToken, requireRole("super_admin", "org_admin", "soc_analyst"), async (req, res) => {
  try {
    const { title, description, severity, alert_id, incident_id, mitre_techniques, mitre_tactics } = req.body;
    ...
    logAudit(req.user!.id, req.user!.username, "CREATE_CASE", "cases", `Created case: ${title}`, req.ip || "", getOrgId(req.user!));
    res.json(c);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
app.put("/api/cases/:id", authenticateToken, requireRole("super_admin", "org_admin", "soc_analyst"), async (req, res) => {
  try {
    const { status, severity, root_cause, resolution, lessons_learned, assigned_to } = req.body;
    let c = await caseManager.getCase(parseInt(req.params.id));
    if (!c) { res.status(404).json({ error: "Case not found." }); return; }
    if (status && status !== c.status) {
      c = await caseManager.updateCaseStatus(c.id, status, req.user!.id, req.body.notes);
    }
    if (c) {
    ...
    logAudit(req.user!.id, req.user!.username, "UPDATE_CASE", "cases", `Updated case #${req.params.id}`, req.ip || "", getOrgId(req.user!));
    res.json(c);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
app.post("/api/cases/:id/assign", authenticateToken, requireRole("super_admin", "org_admin"), async (req, res) => {
  try {
    const { analyst_id } = req.body;
    if (!analyst_id) { res.status(400).json({ error: "analyst_id is required." }); return; }
    const c = await caseManager.assignCase(parseInt(req.params.id), analyst_id, req.user!.id);
    if (!c) { res.status(404).json({ error: "Case not found." }); return; }
    res.json(c);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
...
app.post("/api/cases/:id/timeline", authenticateToken, requireRole("super_admin", "org_admin", "soc_analyst"), async (req, res) => {
  try {
    const { event_type, description, metadata } = req.body;
    if (!event_type || !description) { res.status(400).json({ error: "event_type and description are required." }); return; }
    const event = await caseManager.addTimelineEvent(parseInt(req.params.id), event_type, description, metadata || {}, req.user!.id);
    res.json(event);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
app.get("/api/cases/sla/status", authenticateToken, requireRole("super_admin", "org_admin"), (req, res) => {
...
app.post("/api/copilot/runbooks/suggest", authenticateToken, (req, res) => {
  ...
  const suggestions = caseManager.suggestRunbooks(severity || "MEDIUM", context, mitre_technique);
...
app.get("/api/threat-intel/mitre/technique/:id", authenticateToken, (req, res) => {
...
app.post("/api/threat-intel/lookup", authenticateToken, async (req, res) => {
  try {
    ...
    if (!indicator_type || !value) { res.status(400).json({ error: "indicator_type and value are required." }); return; }
    const results = await threatIntelService.lookupIndicator(indicator_type, value);
    res.json({ results });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
// ========================
app.get("/api/copilot/evaluation/report", authenticateToken, requireRole("super_admin", "org_admin"), (req, res) => {
  const tenantId = getOrgId(req.user!) || 1;
...
app.get("/api/copilot/evaluation/recent", authenticateToken, (req, res) => {
  const tenantId = getOrgId(req.user!) || 1;
  const limit = parseInt(req.query.limit as string) || 50;
  res.json({ evaluations: aiEvaluationService.getRecentEvaluations(tenantId, limit) });
});
...
app.get("/api/copilot/boundaries/enforce", authenticateToken, (req, res) => {
  ...
  if (!prompt && !response) { res.status(400).json({ error: "prompt or response query parameter required" }); return; }
...
app.get("/api/health-monitor/alerts/unacknowledged", authenticateToken, (req, res) => {
  res.json(healthMonitorService.getUnacknowledgedAlerts());
});

app.post("/api/health-monitor/alerts/:id/acknowledge", authenticateToken, requireRole("super_admin", "org_admin", "soc_analyst"), (req, res) => {
...
app.post("/api/health-monitor/check", authenticateToken, requireRole("super_admin", "org_admin"), async (req, res) => {
  const results = await healthMonitorService.runAllChecks();
  const services: Record<string, any> = {};
  for (const [name, health] of results) { services[name] = health; }
...
// DISASTER RECOVERY
// ========================
app.get("/api/dr/status", authenticateToken, requireRole("super_admin", "org_admin"), (req, res) => {
  res.json(disasterRecoveryService.getStatus());
});

app.get("/api/dr/backups", authenticateToken, requireRole("super_admin", "org_admin"), (req, res) => {
  const limit = parseInt(req.query.limit as string) || 50;
  res.json(disasterRecoveryService.getBackupHistory(limit));
});

app.post("/api/dr/backup/postgres", authenticateToken, requireRole("super_admin"), async (req, res) => {
  const result = await disasterRecoveryService.backupPostgres();
  res.json(result);
});

app.post("/api/dr/backup/qdrant", authenticateToken, requireRole("super_admin"), async (req, res) => {
  const result = await disasterRecoveryService.backupQdrant();
  res.json(result);
});

app.post("/api/dr/backup/minio", authenticateToken, requireRole("super_admin"), async (req, res) => {
  const result = await disasterRecoveryService.backupMinIO();
  res.json(result);
});

app.post("/api/dr/backup/sqlite", authenticateToken, requireRole("super_admin", "org_admin"), async (req, res) => {
  const result = await disasterRecoveryService.backupSQLite();
  res.json(result);
});

app.post("/api/dr/backup/all", authenticateToken, requireRole("super_admin"), async (req, res) => {
  const results = await disasterRecoveryService.runAllBackups();
  res.json({ backups: results });
});

app.post("/api/dr/retention/enforce", authenticateToken, requireRole("super_admin"), (req, res) => {
  const result = disasterRecoveryService.enforceRetention();
  res.json(result);
});

app.get("/api/dr/recovery-plans", authenticateToken, requireRole("super_admin", "org_admin"), (req, res) => {
...
app.get("/api/reports/export/:type", authenticateToken, async (req, res) => {
  try {
    ...
    const anomalies = await dbAll("SELECT * FROM anomalies ORDER BY id DESC LIMIT 500");
    const incidents = await dbAll("SELECT * FROM incidents ORDER BY id DESC LIMIT 500");
    const totalEvents = await dbGet<{ total: number }>("SELECT COUNT(*) as total FROM events");
    const auditLogs = await dbAll("SELECT * FROM audit_logs ORDER BY timestamp DESC LIMIT 500");
    ...
    if (type === "json") {
      ...
      if (dataset === "anomalies") { jsonPayload = { anomalies }; filename = "aegis_anomalies.json"; }
      else if (dataset === "incidents") { jsonPayload = { incidents }; filename = "aegis_incidents.json"; }
      else if (dataset === "audit") { jsonPayload = { audit_logs: auditLogs }; filename = "aegis_audit_logs.json"; }
      ...
      if (dataset === "incidents") {
        csv = "ID,Title,Status,Severity,Detection Method,Z-Score,iForest,Hybrid Score,Anomaly ID,Detection Time,Root Cause,AI Diagnosis,Resolution,Recommended Action\n";
        ...
        csv = "ID,User ID,Username,Action,Resource,Details,Timestamp,IP Address\n";
        ...
        csv = "ID,Timestamp,Z-Score,iForest Score,EWMA Score,Hybrid Score,Hybrid Severity,Detection Method,Event Count,Source Entropy,Burst Ratio,Status,Severity,Diagnosis\n";
...
async function startServer() {
  ...
  console.log("[Health] Service health monitoring started (PostgreSQL, Qdrant, MinIO, Gemini, Copilot, SQLite)");
