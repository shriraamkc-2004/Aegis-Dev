...
export function maskPii(
  ...
  if (valueType === "username") {
    ...
    return (
      masked[0] + "*".repeat(masked.length - 2) + masked[masked.length - 1]
    );
  }
  ...
  masked = masked.replace(ipv6Regex, (match) => {
    const parts = match.split(":");
    return `${parts.slice(0, 3).join(":")}:${parts
      .slice(3)
      .map(() => "xxxx")
      .join(":")}`;
  });
...
export async function classifyAnomalyThreat(
  ...
  try {
    ...
    for (const event of recentEvents) {
      ...
      if (
        etLower.includes("fail") ||
        etLower.includes("auth") ||
        etLower.includes("login") ||
        etLower.includes("brute") ||
        srcLower.includes("fail") ||
        srcLower.includes("auth") ||
        srcLower.includes("login") ||
        srcLower.includes("brute") ||
        ordLower.includes("fail") ||
        ordLower.includes("auth") ||
        ordLower.includes("login") ||
        ordLower.includes("brute")
      ) {
        if (
          etLower.includes("fail") ||
          ordLower.includes("fail") ||
      ...
      if (
        etLower.includes("exfil") ||
        etLower.includes("transfer") ||
        etLower.includes("outbound") ||
        etLower.includes("export") ||
        etLower.includes("download") ||
        srcLower.includes("exfil") ||
        srcLower.includes("transfer") ||
        srcLower.includes("outbound") ||
        srcLower.includes("export") ||
        srcLower.includes("download") ||
        ordLower.includes("exfil") ||
        ordLower.includes("transfer") ||
        ordLower.includes("outbound") ||
        ordLower.includes("export") ||
        ordLower.includes("download")
      ) {
        largeTransfers++;
      }
      ...
      if (
        etLower.includes("privileged") ||
        etLower.includes("config") ||
        etLower.includes("admin") ||
        etLower.includes("policy") ||
        etLower.includes("sensitive") ||
        etLower.includes("insider") ||
        srcLower.includes("privileged") ||
        srcLower.includes("config") ||
        srcLower.includes("admin") ||
        srcLower.includes("policy") ||
        srcLower.includes("sensitive") ||
        srcLower.includes("insider") ||
        ordLower.includes("privileged") ||
        ordLower.includes("config") ||
        ordLower.includes("admin") ||
        ordLower.includes("policy") ||
        ordLower.includes("sensitive") ||
        ordLower.includes("insider")
      ) {
        privilegedActions++;
      }
      ...
      if (
        etLower.includes("heartbeat") ||
        etLower.includes("drop") ||
        etLower.includes("miss") ||
        etLower.includes("failure") ||
        etLower.includes("inactive") ||
        etLower.includes("service_fail") ||
        srcLower.includes("heartbeat") ||
        srcLower.includes("drop") ||
        srcLower.includes("miss") ||
        srcLower.includes("failure") ||
        srcLower.includes("inactive") ||
        srcLower.includes("service_fail") ||
        ordLower.includes("heartbeat") ||
        ordLower.includes("drop") ||
        ordLower.includes("miss") ||
        ordLower.includes("failure") ||
        ordLower.includes("inactive") ||
        ordLower.includes("service_fail")
      ) {
        serviceDrops++;
      }
    }
    ...
    console.error(
      `[ThreatClassifier TS Error] Database check failed: ${e.message}`,
    );
  }
...
export async function triggerAnomalyDiscordAlert(
  ...
  if (!webhookUrl || webhookUrl.trim() === "") {
    console.log(
      `[Discord Alert] No webhook URL configured. Anomaly #${anomalyId} skipped.`,
    );
    return false;
  }
  ...
  try {
    ...
    if (!anomaly) {
      console.error(
        `[Discord Alert Error] No anomaly row found for ID: ${anomalyId}`,
      );
      return false;
    }
    ...
    if (!incident) {
      console.error(
        `[Discord Alert Error] No incident row found for anomaly ID: ${anomalyId}`,
      );
      return false;
    }
    ...
    const hybridScore =
      anomaly.hybrid_score !== null ? anomaly.hybrid_score : 0.0;
    const zScore = anomaly.z_score !== null ? anomaly.z_score : 0.0;
    const iforestScore =
      anomaly.iforest_score !== null ? anomaly.iforest_score : 0.0;
    ...
    const possibleThreat = maskPii(
      anomaly.possible_threat || "Suspicious Activity",
    );
    const threatConfidence =
      anomaly.threat_confidence !== null ? anomaly.threat_confidence : 65.0;
    ...
    const fields = [
      ...
      { name: "Possible Threat", value: possibleThreat, inline: true },
      {
        ...
        value: `${threatConfidence.toFixed(1)}%`,
        inline: true,
      },
      ...
      {
        name: "Baseline Mean",
        value: `${mean.toFixed(2)} orders/sec`,
        inline: true,
      },
    ...
    if (mode === "resolved") {
      ...
      const isCriticalRetryFail =
        severity === "CRITICAL" &&
        (!incident.gemini_summary || incident.gemini_summary.trim() === "") &&
        incident.ai_diagnosis &&
        incident.ai_diagnosis.includes("fallback");

      if (
        isCriticalRetryFail ||
        incident.resolution_status === "Escalated" ||
        incident.mitigation_summary?.includes("escalat") ||
        incident.agent_summary?.includes("escalat")
      ) {
      ...
      fields.push(
        ...
        { name: "Recommendation", value: recommendationString, inline: false },
        {
          ...
          value: maskPii(
            incident.root_cause ||
              `Traffic spike of ${orderCount} events/sec from main nodes.`,
          ),
          inline: false,
        },
        {
          name: "Mitigation",
          value: maskPii(incident.mitigation_summary || mitigation),
          inline: false,
        },
      );
    ...
    if (mode === "resolved" && incident.discord_message_id) {
      console.log(
        `[Discord Alert] Updating existing alert message ${incident.discord_message_id} to closed state...`,
      );
      ...
      if (response.ok) {
        console.log(
          `[Discord Alert] Successfully closed incident #${anomalyId} alert on Discord.`,
        );
        ...
        console.error(
          `[Discord Alert Error] PATCH message returned status: ${response.status}`,
        );
        return false;
      }
    } else {
      console.log(
        `[Discord Alert] Dispatches new alert message for anomaly #${anomalyId}...`,
      );
      ...
      if (response.ok) {
        ...
        console.log(
          `[Discord Alert] Created alert message ID: ${messageId} on Discord.`,
        );
        return true;
      } else {
        console.error(
          `[Discord Alert Error] POST message returned status: ${response.status}`,
        );
        return false;
      }
    }
    ...
    console.error(
      `[Discord Alert TS Error] Failed sending Discord webhook: ${e.message}`,
    );
    return false;
  }
}
