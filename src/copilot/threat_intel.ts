...
export interface ThreatIntelResult {
  source: "mitre_attack" | "virustotal" | "abuseipdb" | "alienvault_otx" | "custom";
...
export const MITRE_TECHNIQUES: MitreTechnique[] = [
  {
    ...
    mitigation: "Implement account lockout policies, enforce strong passwords, enable MFA.",
    detection: "Monitor for multiple failed authentication attempts from single or distributed sources.",
    subtechniques: [
  ...
  },
  {
    ...
    mitigation: "Deploy DDoS mitigation services, implement rate limiting, use CDN filtering.",
    detection: "Monitor for traffic volume spikes, unusual source IP distribution, and protocol anomalies.",
    subtechniques: [
  ...
  },
  {
    ...
    mitigation: "Monitor outbound data transfers, implement DLP, restrict egress traffic.",
    detection: "Detect large outbound transfers, unusual protocols, or connections to known-bad IPs.",
  },
  {
    id: "T1078", name: "Valid Accounts", tactic: "persistence",
    description: "Adversaries may obtain and abuse credentials of existing accounts.",
    mitigation: "Enforce least privilege, rotate credentials, monitor for anomalous account usage.",
    detection: "Monitor logins from unusual locations, times, or devices. Track privilege escalation.",
    subtechniques: [
  ...
  },
  {
    ...
    mitigation: "Restrict script execution, use application whitelisting, monitor process creation.",
    detection: "Monitor process execution for unusual scripts, encoded commands, or suspicious parent-child relationships.",
  },
  {
    id: "T1566", name: "Phishing", tactic: "initial-access",
    description: "Adversaries may send phishing messages to gain access to victim systems.",
    mitigation: "Train users on phishing awareness, implement email filtering, use DMARC/SPF/DKIM.",
    detection: "Monitor email logs for suspicious senders, links, and attachments.",
    subtechniques: [
  ...
  },
  {
    ...
    mitigation: "Restrict remote access, implement network segmentation, monitor lateral connections.",
    detection: "Monitor for unusual remote desktop, SSH, or WinRM connections between internal hosts.",
  },
  {
    id: "T1055", name: "Process Injection", tactic: "defense-evasion",
    description: "Adversaries may inject code into processes to evade detection.",
    mitigation: "Use EDR solutions, enable ASLR, monitor for suspicious memory allocations.",
    detection: "Monitor for unusual process memory access patterns and DLL loading.",
  },
  {
    id: "T1071", name: "Application Layer Protocol", tactic: "command-and-control",
    description: "Adversaries may use application layer protocols for C2 communications.",
    mitigation: "Inspect encrypted traffic, block unauthorized protocols, monitor DNS queries.",
    detection: "Detect beaconing patterns, unusual DNS queries, or HTTP requests to suspicious domains.",
  },
  {
    id: "T1486", name: "Data Encrypted for Impact", tactic: "impact",
    description: "Adversaries may encrypt data on target systems to interrupt availability (ransomware).",
    mitigation: "Maintain offline backups, implement file integrity monitoring, restrict write access.",
    detection: "Monitor for mass file modifications, unusual encryption operations, or ransom notes.",
  },
];
...
const ANOMALY_TO_MITRE: AnomalyMapping[] = [
  {
    patterns: ["brute_force", "brute force", "failed_login", "authentication", "failed_login"],
    techniques: ["T1110"],
    tactics: ["TA0006"],
  ...
  {
    patterns: ["insider", "privileged", "insider_threat", "unauthorized_access"],
    techniques: ["T1078"],
    tactics: ["TA0003"],
...
export class ThreatIntelService {
  ...
  mapAnomalyToMitre(
    ...
    const combined = (anomalyDescription + " " + (eventType || "")).toLowerCase();
    ...
    return {
      techniques,
      tactics,
      mapping_description: descriptions.join("; ") || "No specific MITRE ATT&CK mapping found",
    };
  }
  ...
  private async queryVirusTotal(
    ...
    let url = "";
    if (indicatorType === "ip") url = `https://www.virustotal.com/api/v3/ip_addresses/${value}`;
    else if (indicatorType === "domain") url = `https://www.virustotal.com/api/v3/domains/${value}`;
    else if (indicatorType === "hash") url = `https://www.virustotal.com/api/v3/files/${value}`;
    else return null;
    ...
    return {
      ...
      severity: malicious > 10 ? "CRITICAL" : malicious > 3 ? "HIGH" : malicious > 0 ? "MEDIUM" : "LOW",
      metadata: {
