...
export const detectionQueue = new Queue<DetectionJobData>(
  "detection-triggers",
  {
    connection: bullConnection,
    defaultJobOptions: {
...
export const eventWorker = new Worker<EventJobData>(
  ...
  async (job: Job<EventJobData>) => {
    const { event_type, source, timestamp, organization_id, raw_data } =
      job.data;
...
export const detectionWorker = new Worker<DetectionJobData>(
  ...
  async (job: Job<DetectionJobData>) => {
    ...
    console.log(
      `Running detection cycle for org ${organization_id} (reason: ${trigger_reason})`,
    );
